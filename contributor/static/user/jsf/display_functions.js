...
//						console.log(text2);
				...
				output += "<p>";
				output += "Created by: ";
				output += xmlDoc2.getElementsByTagName(type)[0].getAttribute("user");
				output += "</p>";
...
//				output += '<p><a href="http://freestreetmap.org/browse/' + type + '/' + id + '/history/" target="_blank">Show History</a></p>';
				output += '<p><a href="http://freestreetmap.org/' + type + '/' + id + '/history/" target="_blank">Show History</a></p>';
...
function getChangesetsByUser (display_name, date) {

	console.log("date = " + date);

	var count = 0;

	document.title = 'fosm - changesets by user - ' + display_name;

	var output = '<p class="user-heading">' + display_name + '</p>';
...
//				console.log(text);
				...
				output += "<p>";
				output += "Changesets by user:"
				output += "</p>";

				var t = xmlDoc.getElementsByTagName('changeset');

				for (var i = 0; i < t.length; i++) {

					output += '<a href="/changeset/';
					output += t[i].getAttribute("id");
					...
					date = t[i].getAttribute("created_at");
	...
	var url = fosm_url +  'changesets?display_name=' + display_name;
	xhttp.open("GET", url, true);
	xhttp.send();
...
function getUserDetails (display_name) {

	document.title = 'fosm - user details - ' + display_name;
