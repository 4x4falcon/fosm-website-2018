...
var fosm_url = 'http://fosm.org/api/0.6/';
...
//				console.log(text);
				...
				output += "</p>";
				output += "<p>";
				output += 'Changeset: <a href="/changeset/' + xmlDoc.getElementsByTagName(type)[0].getAttribute("changeset") +  '" target="_blank">' + xmlDoc.getElementsByTagName(type)[0].getAttribute("changeset") + '</a>';
				output += "</p>";
				output += "<p>";
...
//				output += '<p><a href="http://api.freestreetmap.org/' + type + '/' + id + '/history/" target="_blank">Show History</a></p>';
				output += '<p><a href="http://fosm.org/' + type + '/' + id + '/history/" target="_blank">Show History</a></p>';
...
//				console.log(text);
				...
				output += "</p>";
				output += "<p>";
				output += 'Changeset: <a href="/changeset/' + xmlDoc.getElementsByTagName(type)[0].getAttribute("changeset");
...
//				output += '<p><a href="http://api.freestreetmap.org/' + type + '/' + id + '/history/" target="_blank">Show History</a></p>';
				...
				if (xmlDoc.getElementsByTagName("nd")[0]) {
					...
					for (var i = 0; i < nd.length; i++) {
						output += "id: " + '<a href="/node/' + nd[i].getAttribute('ref') + '" target="_blank">' + nd[i].getAttribute('ref') + '</a><br />';
...
// created
						...
						for (var k = 0; k < u.length; k++) {
							output += "Node: " + '<a href="/node/' + u[k].getAttribute('id') + '" target="_blank">' + u[k].getAttribute('id') + '</a><br />';
						}
						var u = t[i].getElementsByTagName("way");
						for (var k = 0; k < u.length; k++) {
							output += "Way: " + '<a href="/way/' + u[k].getAttribute('id') + '" target="_blank">' + u[k].getAttribute('id') + '</a><br />';
						}
						var u = t[i].getElementsByTagName("relation");
						for (var k = 0; k < u.length; k++) {
							output += "Relation: " + '<a href="/relation/' + u[k].getAttribute('id') + '" target="_blank">' + u[k].getAttribute('id') + '</a><br />';
						}
                                        }
                                }
...
function getObjectHistory(type, id) {

	document.title = 'fosm - history - ' + type + ' ' + id;

        console.log ("history  - id = " + id + " type = " + type);

	var title = '<p class="browse-heading">';
	title += capitalizeFirstLetter(type) + ": " + id + " History";
	title += '</p>';
...
// node
			if (type == 'node') {
				...
				if (xmlDoc.getElementsByTagName("node")[0]) {
					...
					for (var i = 0; i < node.length; i++) {
						...
						output += '<div class="history-block">';

						output += "<p>";
						output += "Version: ";// . $item['version'];
						output += node[i].getAttribute("version");
						output += "</p>";

						output += "<p>";
						output += "Latitude: ";							// . $item['lat'];
						output += node[i].getAttribute("lat");
						output += "<br />";
						output += "Longitude: ";						// . $item['lon'];
						output += node[i].getAttribute("lon");
						output += "</p>";

						output += "<p>";
						output += 'Changeset: <a href="/changeset/' + node[i].getAttribute("changeset") +  '" target="_blank">' + node[i].getAttribute("changeset") + '</a>';
						output += "</p>";
						output += "<p>";


						output += "Last edited: ";// . $item['timestamp'];
						output += node[i].getAttribute("timestamp");
						output += "</p>";
						output += "<p>";
						output += "Edited by: ";// . $item['user'];
						output += node[i].getAttribute("user");
						output += "</p>";

						if (node[i].getElementsByTagName("tag")[0]) {
							output += "<p>Tags</p>";
							output += "<p>";

							var tag = node[i].getElementsByTagName("tag");

							for (var j = 0; j < tag.length; j++) {

								output += tag[j].getAttribute("k") + ": " + tag[j].getAttribute("v");
								output += "<br />";
...
// way
			if (type == 'way') {
				...
				if (xmlDoc.getElementsByTagName("way")[0]) {
					...
					for (var i = 0; i < way.length; i++) {
						...
						output += "<p>";
						output += "Version: ";// . $item['version'];
						output += way[i].getAttribute("version");
						output += "</p>";

						output += "<p>";
						output += 'Changeset: <a href="/changeset/' + way[i].getAttribute("changeset") +  '" target="_blank">' + way[i].getAttribute("changeset") + '</a>';
						output += "</p>";
						output += "<p>";

						output += "Last edited: ";// . $item['timestamp'];
						output += way[i].getAttribute("timestamp");
						output += "</p>";
						output += "<p>";
						output += "Edited by: ";// . $item['user'];
						output += way[i].getAttribute("user");
						output += "</p>";

						if (way[i].getElementsByTagName("tag")[0]) {
							output += "<p>Tags</p>";
							output += "<p>";

							var tag = way[i].getElementsByTagName("tag");
						...
						if (way[i].getElementsByTagName("nd")[0]) {
							output += "<p>Nodes</p>";
							output += "<p>";

							var nd = way[i].getElementsByTagName("nd");

							for (var j = 0; j < nd.length; j++) {

								output += "id : " + nd[j].getAttribute("ref");
								output += "<br />";
...
// relation
	...
	var url = fosm_url + type + '/' + id + '/history';
	xhttp.open("GET", url, false);
	xhttp.send();
